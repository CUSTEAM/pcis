...
public class LeaveAlert extends BaseJob{
	...
	public LeaveAlert(ApplicationContext springContext){
	...
	public void doit(){	
		...
		if(today.getTime()>=school_term_begin.getTime() && today.getTime()<=school_term_end.getTime()){
			for(int i=0; i<empl.size(); i++){
				try{
					...
					for(int j=0; j<tmp.size(); j++){
						sb.append(((Map)tmp.get(j)).get("name")+" - "+((Map)tmp.get(j)).get("ClassName")+" - "+((Map)tmp.get(j)).get("student_name")+", 共"+((Map)tmp.get(j)).get("cls")+"節<br>");
					}
					
					
						email=((Map)empl.get(i)).get("Email").toString();
		    			if(!si.validateEmail(email)||email.trim().equals("")||email.trim().length()<10||email==null){  
		    				f.append(empl.get(i).get("cname")+",");
		    				continue;
		    			}
		    			this.sendMail(email, "學生請假待審通知", sb.toString());
			...
			try {
				sam.getDataFinder().exSql("INSERT INTO SYS_SCHEDULE_LOG(subject,note)VALUES('學生請假待審通知','"+a+"<br>"+f+"');");
			} catch (Exception e) {				
				e.printStackTrace();
			} 
		}else{
