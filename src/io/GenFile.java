...
public class GenFile {
    ...
    private static Map spilt(String entrance){
    	Map map=new HashMap();
    	if(entrance.length()==5){
    		System.out.println(entrance.substring(4, 5));
    		map.put("year", Integer.parseInt(entrance.substring(0, 3))+1911);
    		map.put("month", entrance.substring(4, 5));
    	}else{    		
    		System.out.println(entrance.substring(3, 4));
    		map.put("year", Integer.parseInt(entrance.substring(0, 2))+1911);
    		map.put("month", entrance.substring(3, 4));
    	}
    	return map;
    }
